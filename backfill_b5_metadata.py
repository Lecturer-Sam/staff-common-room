#!/usr/bin/env python3
"""Backfill B5 Core-4 DBs (math, science, english, ghanaian) with authentic NaCCA
text from the Upper Primary PDFs. All grabs GRADE-SCOPED to BASIC 5 body slices.
Includes auto-cleaning (CONT'D, page footers, competency leaks) and artifact
detection (reports codes with zero hits in the body before any removal)."""
...
BODIES = {
    'mathematics_B4-B6.pdf': (119132, 192017),
    'science_B4-B6.pdf': (74562, 113742),
    'english_B4-B6.pdf': (148664, 256501),
    'ghanaian_language_B4-B6.pdf': (119392, 170461),
}

JOBS = [
    ('mathematics', 'B5', 'mathematics_B4-B6.pdf', {
        1: '1. NUMBER', 2: '2. ALGEBRA', 3: '3. GEOMETRY AND MEASUREMENT', 4: '4. DATA'}),
    ('science', 'B5', 'science_B4-B6.pdf', {
        1: '1. DIVERSITY OF MATTER', 2: '2. CYCLES', 3: '3. SYSTEMS',
        4: '4. FORCES AND ENERGY', 5: '5. HUMANS AND THE ENVIRONMENT'}),
    ('english-language', 'B5', 'english_B4-B6.pdf', {
        1: '1. ORAL LANGUAGE', 2: '2. READING',
        3: '3. GRAMMAR AND USAGE AT WORD AND PHRASE LEVELS', 4: '4. WRITING',
        5: '5. USING WRITING CONVENTIONS/ GRAMMAR USAGE', 6: '6. EXTENSIVE READING'}),
    ('ghanaian-language', 'B5', 'ghanaian_language_B4-B6.pdf', {
...
for sid, g, pdf, strand_map in JOBS:
    ...
    bak = os.path.join(BK, os.path.basename(path) + '.pre_b5backfill')
    if not os.path.exists(bak):
        shutil.copy2(path, bak)
    db = json.load(open(path))
    # artifact detection: codes with zero occurrences in the B5 body
