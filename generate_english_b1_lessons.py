...
# Load English B1 curriculum DB
with open('/home/user/english_curriculum_db_clean.json') as f:
    cur_db = json.load(f)

print(f"Loaded {len(cur_db)} English B1 indicators")
...
def get_english_activities(ind_code, meta, session_num, total_sessions, day_name, is_revision=False):
    strand = meta.get("strand","")
    sub_strand = meta.get("sub_strand","")
    ind_desc = meta.get("ind_desc","")
    keywords = meta.get("keywords","english, reading, writing")
    # session title
    if is_revision:
    ...
    if "Oral" in strand or day_name=="Monday":
        starter = [
            "Greet learners warmly in English and Mother Tongue. Sing 'Good Morning' with actions and smiles.",
        ...
        plenary = [
            ...
            "Homework: 'Practise telling your family what we learned, in English and Mother Tongue.'",
...
for l in lessons_raw:
    ...
    act=get_english_activities(code, meta, session_num, total_sessions, l["day"], l.get("is_revision",False))
    # RPK mapping
    strand_name=meta.get("strand","English Language")
    rpk_map={
        ...
        "Using Writing Conventions / Grammar Usage": "Learners use simple sentences orally in everyday interaction and are familiar with basic classroom English.",
    ...
    enriched.append({
        ...
        "keywords": meta.get("keywords","english, oral, reading, writing"),
        "rpk": rpk,
        "starter": act["starter"],
        "main": act["main"],
        "plenary": act["plenary"],
...
print(f"Enriched {len(enriched)} English B1 lessons")
with open("/home/user/english_lessons_enriched.json","w",encoding="utf-8") as out:
    json.dump(enriched,out,indent=2,ensure_ascii=False)
print("Saved to /home/user/english_lessons_enriched.json")
