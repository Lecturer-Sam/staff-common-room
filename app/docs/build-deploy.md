...
| `yarn deploy:rules` | Deploy `firestore.rules` + indexes (needs Firebase CLI login; add `--project <id>` if unresolved) |
...
**Not** handled by Vercel. After editing `firestore.rules` or
`firestore.indexes.json` — **this is part of every release that touches
either file** (e.g. the Phase-1 `subscriptions` rules):

```
yarn deploy:rules      # = firebase deploy --only firestore:rules,firestore:indexes
```
