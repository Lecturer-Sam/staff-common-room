...
function Skeleton({ width = 'w-full', height = 'h-4', className = '' }) {
  return (
    <div
      className={`hearth-skeleton ${width} ${height} ${className}`}
    />
  )
}
...
function SkeletonCard() {
  return (
    <div className="hearth-skeleton-card">
...
function SkeletonList({ count = 4 }) {
  return (
    <ul className="hearth-skeleton-list">
...
function SkeletonGrid({ count = 6, cols = 'sm:grid-cols-2 lg:grid-cols-3' }) {
  return (
    <div className={`hearth-skeleton-grid ${cols}`}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="hearth-skeleton-card">
