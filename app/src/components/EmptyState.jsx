...
const icons = {
  feed: (
    <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="hearth-empty__icon">
...
export default function EmptyState({ icon = 'default', title, body, action }) {
  ...
  return (
    <div className="hearth-empty">
      <div className="hearth-empty__illustration">{illustration}</div>
      <p className="hearth-empty__title">{title}</p>
      {body && (
        <p className="hearth-empty__body">{body}</p>
      )}
      {action && (
        <div className="hearth-empty__action">
          {action.to ? (
