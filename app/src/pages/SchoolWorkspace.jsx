...
import { listSchoolGenerations } from '../lib/generatedMaterials'
...
export default function SchoolWorkspace() {
  ...
  useEffect(() => {
    ...
    // Documents produced with the Generate screen. Equality-only, like the
    // queries above, so no composite index is needed.
    listSchoolGenerations(schoolId, LIST_CAP)
      .then((list) => active && setGenerated(list))
      .catch(() => active && setGenerated([]))
    return () => {
      active = false
    }
  }, [schoolId])
  ...
  return (
    <div>
      ...
      <section className="mb-8">
        <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-slate-500">
          Materials generated by your teachers
        </h2>
        {!generated ? (
          <p className="text-sm text-slate-400">Loading…</p>
        ) : generated.length === 0 ? (
          <p className="text-sm text-slate-500">
            Nothing generated yet — teachers can produce schemes and records of
            work from the{' '}
            <Link
              to="/portal/materials"
              ...
              {generated.map((g) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-slate-800">
                      {g.kind === 'scheme'
                        ...
                        : 'Record of Work'}
                      {' · '}
                      {(g.grade ?? '').replace('B', 'Basic ')}
                      {g.term ? ` · Term ${g.term}` : ' · Full year'}
                      {g.isZip && (
                        <Badge className="ml-1">all subjects</Badge>
                      )}
                    </p>
                  ...
                  <span className="shrink-0 text-xs text-slate-400">
                    {g.authorName || 'A teacher'} · {fmtDate(g.createdAt)}
                  </span>
                </li>
              ))}
            </ul>
