#!/usr/bin/env python3
"""Backfill B7 Core-4 DBs (math, science, english, ghanaian) with authentic NaCCA CCP text.
All grabs GRADE-SCOPED to BASIC 7 body slices of the CCP PDFs."""
...
def code_rx(code):
    # CCP PDFs sometimes print codes with a /JHSn infix: B7/JHS1.1.1.1.1
    parts = code.split('.')
...
BODIES = {
    'mathematics_CCP_B7-B9_draft.pdf': (46835, 126618),
    'science_CCP_B7-B9.pdf': (73701, 143098),
    'english_CCP_B7-B9.pdf': (72178, 118892),
    'ghanaian_language_CCP_B7-B9.pdf': (74627, 109353),
}

JOBS = [
    ('mathematics', 'B7', 'mathematics_CCP_B7-B9_draft.pdf', {
        1: '1. NUMBER', 2: '2. ALGEBRA', 3: '3. GEOMETRY AND MEASUREMENT', 4: '4. DATA'}),
    ('science', 'B7', 'science_CCP_B7-B9.pdf', {
        1: '1. DIVERSITY OF MATTER', 2: '2. CYCLES', 3: '3. SYSTEMS',
        4: '4. FORCES AND ENERGY', 5: '5. HUMANS AND THE ENVIRONMENT'}),
    ('english-language', 'B7', 'english_CCP_B7-B9.pdf', {
        1: '1. ORAL LANGUAGE (LISTENING AND SPEAKING)', 2: '2. READING',
        3: '3. GRAMMAR USAGE', 4: '4. WRITING', 5: '5. LITERATURE'}),
    ('ghanaian-language', 'B7', 'ghanaian_language_CCP_B7-B9.pdf', {
...
for sid, g, pdf, strand_map in JOBS:
    ...
    bak = os.path.join(BK, os.path.basename(path) + '.pre_b7backfill')
...
for sid, g, n, i, c, s, misses, zero_hits in report:
    ...
    if zero_hits:
        print(f"   ? ZERO hits in B7 body (artifact candidates): {zero_hits}")
    if misses:
