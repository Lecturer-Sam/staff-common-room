#!/usr/bin/env python3
"""Backfill B4 Creative Arts, History, RME DBs with authentic NaCCA text.
All grabs GRADE-SCOPED to verified BASIC 4 body slices."""
...
def clean_desc(d):
    d = re.sub(r'(©\s*NaCCA.*|CONTENT STANDARDS.*|SUBJECT SPECIFIC.*|INDICATORS AND EXEMPLARS.*)$', '', d).strip(' .|')
    while True:
        m = COMP_PAT.search(d)
        if not m:
            break
...
BODIES = {
    'creative_arts_B4-B6.pdf': (86865, 156231),
    'history.pdf': (76162, 87754),
    'rme_B1-B6.pdf': (79797, 92252),
}

JOBS = [
    ('creative-arts', 'B4', 'creative_arts_B4-B6.pdf',
     {1: '1. VISUAL ARTS', 2: '2. PERFORMING ARTS'}),
    ('history', 'B4', 'history.pdf',
     {1: '1. HISTORY AS A SUBJECT', 2: '2. MY COUNTRY GHANA', 3: '3. EUROPEANS IN GHANA',
      4: '4. COLONISATION AND DEVELOPMENTS UNDER COLONIAL RULE IN GHANA',
      6: '6. INDEPENDENT GHANA'}),
    ('rme', 'B4', 'rme_B1-B6.pdf',
     {1: '1. GOD, HIS CREATION AND ATTRIBUTES',
      2: '2. RELIGIOUS PRACTICES AND THEIR MORAL IMPLICATIONS',
      3: '3. RELIGIOUS LEADERS', 4: '4. THE FAMILY AND COMMUNITY',
...
for sid, g, pdf, strand_map in JOBS:
    ...
    bak = os.path.join(BK, os.path.basename(path) + '.pre_b4r3backfill')
