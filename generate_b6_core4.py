#!/usr/bin/env python3
"""Generate 180 full-year daily lesson plans for Basic 6 Core-4 subjects:
Mathematics, Science, English, Ghanaian Language. Uses verified B6 DBs.
...
Output: /home/user/{subject}_b6_lessons_enriched.json
"""
import json
...
subjects = [
    {"key": "math", "sid": "mathematics", "name": "Mathematics",
     "plan": [("Monday", "B6.1"), ("Tuesday", "B6.2"), ("Wednesday", "B6.3"),
              ("Thursday", "B6.4"), ("Friday", "B6.1")]},
    {"key": "science", "sid": "science", "name": "Integrated Science",
     "plan": [("Monday", "B6.1"), ("Tuesday", "B6.2"), ("Wednesday", "B6.3"),
              ("Thursday", "B6.4"), ("Friday", "B6.5")]},
    {"key": "english", "sid": "english-language", "name": "English Language",
     ...
     "patterns": [
         [("Monday", "B6.1"), ("Tuesday", "B6.2"), ("Wednesday", "B6.3"), ("Thursday", "B6.4"), ("Friday", "B6.6")],
         [("Monday", "B6.1"), ("Tuesday", "B6.2"), ("Wednesday", "B6.3"), ("Thursday", "B6.4"), ("Friday", "B6.5")],
         [("Monday", "B6.1"), ("Tuesday", "B6.2"), ("Wednesday", "B6.1"), ("Thursday", "B6.4"), ("Friday", "B6.5")],
     ]},
    {"key": "ghanaian", "sid": "ghanaian-language", "name": "Ghanaian Language",
     "plan": [("Monday", "B6.1"), ("Tuesday", "B6.2"), ("Wednesday", ["B6.3", "B6.6"]),
              ("Thursday", "B6.4"), ("Friday", "B6.5")]},  # odd wk: Writing, even: Extensive
]

GH_STRAND_NAMES = {
    "B6.1": "1. NTI - Listening and Speaking / Oral Language",
    "B6.2": "2. AKENKAN - Reading",
    "B6.3": "3. ATWER\u0190 - Writing",
    "B6.4": "4. ATWER\u0190 - Composition Writing",
    "B6.5": "5. ATWER\u0190 HO NHYEHY\u0190E / Writing Conventions",
    "B6.6": "6. AKENKAN TR\u0190W - Extensive Reading",
}

RPK = {
    "math": "Learners worked with numbers up to 1,000,000, fractions, integers, shapes and data in Basic 1-5 and use money and measures daily.",
    "science": "Learners classified living things, explored materials, cycles, systems, energy and their environment through Basic 1-5 practical activities.",
    "english": "Learners can hold conversations, read varied passages and write structured paragraphs from Basic 1-5.",
    "ghanaian": "Learners speak the local language daily and built songs, reading, composition and grammar skills through Basic 1-5.",
}
...
def build_one(subj):
    path = f"{DBDIR}/{subj['sid']}_B6_curriculum_db_clean.json"
    with open(path) as f:
        cur_db = json.load(f)
    ...
    out = f"/home/user/{subj['key']}_b6_lessons_enriched.json"
