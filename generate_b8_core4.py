#!/usr/bin/env python3
"""Generate 180 full-year daily lesson plans for Basic 8 Core-4 subjects (CCP/JHS):
Mathematics, Science, English, Ghanaian Language. Uses verified B8 DBs (CCP).
...
Output: /home/user/{subject}_b8_lessons_enriched.json
"""
import json
...
subjects = [
    {"key": "math", "sid": "mathematics", "name": "Mathematics",
     "plan": [("Monday", "B8.1"), ("Tuesday", "B8.2"), ("Wednesday", "B8.3"),
              ("Thursday", "B8.4"), ("Friday", "B8.1")]},
    {"key": "science", "sid": "science", "name": "Integrated Science",
     "plan": [("Monday", "B8.1"), ("Tuesday", "B8.2"), ("Wednesday", "B8.3"),
              ("Thursday", "B8.4"), ("Friday", "B8.5")]},  # 5 strands = 5 days
    {"key": "english", "sid": "english-language", "name": "English Language",
     "plan": [("Monday", "B8.1"), ("Tuesday", "B8.2"), ("Wednesday", "B8.3"),
              ("Thursday", "B8.4"), ("Friday", "B8.5")]},  # Friday = Literature reading period
    {"key": "ghanaian", "sid": "ghanaian-language", "name": "Ghanaian Language",
     "plan": [("Monday", "B8.1"), ("Tuesday", "B8.2"), ("Wednesday", ["B8.3", "B8.6"]),
              ("Thursday", "B8.4"), ("Friday", ["B8.5", "B8.6"])]},  # odd wk: Writing, even: Extensive
]

GH_STRAND_NAMES = {
    "B8.1": "1. CUSTOMS AND INSTITUTIONS",
    "B8.2": "2. LISTENING AND SPEAKING",
    "B8.3": "3. READING",
    "B8.4": "4. LANGUAGE AND USAGE",
    "B8.5": "5. COMPOSITION WRITING",
    "B8.6": "6. LITERATURE",
}

RPK = {
    "math": "Learners worked with numbers, sets, fractions, decimals, integers, ratios, shapes and data through primary school and JHS 1 (B1-B7).",
    "science": "Learners studied mixtures, living things, cycles, systems, energy and their environment through B1-B7 practical activities.",
    "english": "Learners can hold conversations, read varied texts and write structured paragraphs; they know songs, folktales, poems, drama and stories from B1-B7.",
    "ghanaian": "Learners speak the local language daily and know its customs, songs, proverbs, folktales and literature from B1-B7 and home."
}
...
def build_one(subj):
    path = f"{DBDIR}/{subj['sid']}_B8_curriculum_db_clean.json"
    with open(path) as f:
        cur_db = json.load(f)
    ...
    out = f"/home/user/{subj['key']}_b8_lessons_enriched.json"
