#!/usr/bin/env python3
"""Generate 180 full-year daily lesson plans for Basic 3 remaining subjects:
Creative Arts, History, RME. Uses verified B3 DBs (authentic NaCCA text).
Output: /home/user/{creative_arts,history,rme}_b3_lessons_enriched.json
"""
import json
...
subjects = [
    {"key": "creative_arts", "sid": "creative-arts", "name": "Creative Arts",
     "week_plan": [("Monday", "B3.1", 1), ("Tuesday", "B3.2", 2), ("Wednesday", "B3.1", 1),
                   ("Thursday", "B3.2", 2), ("Friday", "B3.1", 1)]},
    {"key": "history", "sid": "history", "name": "History",
     "week_plan": None},  # global indicator cycle: 7 indicators
    {"key": "rme", "sid": "rme", "name": "Religious and Moral Education",
     "week_plan": [("Monday", "B3.1", 1), ("Tuesday", "B3.2", 2), ("Wednesday", "B3.3", 3),
                   ("Thursday", "B3.4", 4), ("Friday", "B3.1", 1)]},
]

RPK = {
    "creative_arts": "Learners sing, dance, draw, colour, mould and role-play informally at home; they explored Visual and Performing Arts in Basic 1 and 2.",
    "history": "Learners listened to stories from elders, know some traditions and festivals, and learned about the people and major historical locations of Ghana in Basic 1 and 2.",
    "rme": "Learners pray and worship with their families (Christian, Islamic, Traditional) and learned about God's creation, worship and religious leaders in Basic 1 and 2.",
}
...
def build_one(subj):
    ...
    if subj["week_plan"] is None:
        # global round-robin over all indicators (thin curricula, e.g. History's 7)
        ...
        strand_iters = {sc: cycle(strand_groups[sc]) for sc in strand_groups}  # pre-built!
        for term in (1, 2, 3):
            for week in range(1, 13):
    ...
    out = f"/home/user/{subj['key']}_b3_lessons_enriched.json"
    ...
    lo = dist.most_common()[-1]
    print(f"OK {subj['name']}: {len(enriched)} lessons -> {out} | spread {top[1]}..{lo[1]} sessions/indicator")
