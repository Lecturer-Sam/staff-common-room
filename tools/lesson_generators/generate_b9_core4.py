#!/usr/bin/env python3
"""Generate 180 full-year daily lesson plans for Basic 9 Core-4 subjects (CCP/JHS):
Mathematics, Science, English, Ghanaian Language. Uses verified B9 DBs (CCP).
...
Output: /home/user/{subject}_b9_lessons_enriched.json
"""
import json
...
subjects = [
    {"key": "math", "sid": "mathematics", "name": "Mathematics",
     "plan": [("Monday", "B9.1"), ("Tuesday", "B9.2"), ("Wednesday", "B9.3"),
              ("Thursday", "B9.4"), ("Friday", "B9.1")]},
    {"key": "science", "sid": "science", "name": "Integrated Science",
     "plan": [("Monday", "B9.1"), ("Tuesday", "B9.2"), ("Wednesday", "B9.3"),
              ("Thursday", "B9.4"), ("Friday", "B9.5")]},  # 5 strands = 5 days
    {"key": "english", "sid": "english-language", "name": "English Language",
     "plan": [("Monday", "B9.1"), ("Tuesday", "B9.2"), ("Wednesday", "B9.3"),
              ("Thursday", "B9.4"), ("Friday", "B9.5")]},  # Friday = Literature reading period
    {"key": "ghanaian", "sid": "ghanaian-language", "name": "Ghanaian Language",
     "plan": [("Monday", "B9.1"), ("Tuesday", "B9.2"), ("Wednesday", ["B9.3", "B9.6"]),
              ("Thursday", "B9.4"), ("Friday", ["B9.5", "B9.6"])]},  # odd wk: Writing, even: Extensive
]

GH_STRAND_NAMES = {
    "B9.1": "1. CUSTOMS AND INSTITUTIONS",
    "B9.2": "2. LISTENING AND SPEAKING",
    "B9.3": "3. READING",
    "B9.4": "4. LANGUAGE AND USAGE",
    "B9.5": "5. COMPOSITION WRITING",
    "B9.6": "6. LITERATURE",
}

RPK = {
    "math": "Learners worked with number systems, algebraic expressions, geometry, trigonometry and data through B1-B8.",
    "science": "Learners studied chemical compounds, living things, cycles, systems, electronics and their environment through B1-B8 practical activities.",
    "english": "Learners can hold conversations, read varied texts and write structured compositions; they know folktales, poems, drama and stories from B1-B8.",
    "ghanaian": "Learners speak the local language daily and know its customs, rites, songs, proverbs, folktales and literature from B1-B8 and home."
}
...
def build_one(subj):
    path = f"{DBDIR}/{subj['sid']}_B9_curriculum_db_clean.json"
    with open(path) as f:
        cur_db = json.load(f)
    ...
    out = f"/home/user/{subj['key']}_b9_lessons_enriched.json"
