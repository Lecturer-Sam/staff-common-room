...
with open("math_b7_lessons_enriched.json") as f:
    lessons = json.load(f)

print(f"Building Word Document for {len(lessons)} Mathematics lessons...")
...
run_meta = meta_p.add_run(
    ...
    "Pacing: 2 periods per week for Strand 1 (Number)\n"
    "1 period per week for Strands 2, 3, and 4 (Algebra, Geometry, Data)"
)
...
for idx, l in enumerate(lessons):
    ...
    style_run(r, 10, bold=True)
    r = id_p.add_run("Mathematics\n")
    style_run(r, 10)
    
    r = id_p.add_run("Class: ")
    style_run(r, 10, bold=True)
    r = id_p.add_run("Basic 7 (JHS 1)    ")
    style_run(r, 10)
    ...
    style_run(r, 10, bold=True)
    r = id_p.add_run(f"NaCCA Mathematics CCP Curriculum B7 (2020)\n")
    style_run(r, 10)
...
output_path = "Basic7_Mathematics_Lesson_Plans_Full_Year.docx"
