#!/usr/bin/env python3
"""Audit C (B8 remaining-6) - Verify RME, Computing, Social Studies, Career Tech, CAD, French B8 lesson JSONs + DOCX lesson-plan files."""
import json, re
...
CODE_RE = re.compile(r'^B8\.\d+\.\d+\.\d+\.\d+$')
...
SUBJECTS = {
    'rme_b8':                  ('curriculum_db/rme_B8_curriculum_db_clean.json',                  'Basic8_RME_Lesson_Plans_Full_Year.docx'),
    'computing_b8':            ('curriculum_db/computing_B8_curriculum_db_clean.json',            'Basic8_Computing_Lesson_Plans_Full_Year.docx'),
    'social_studies_b8':       ('curriculum_db/social-studies_B8_curriculum_db_clean.json',       'Basic8_Social_Studies_Lesson_Plans_Full_Year.docx'),
    'career_technology_b8':    ('curriculum_db/career-technology_B8_curriculum_db_clean.json',    'Basic8_Career_Technology_Lesson_Plans_Full_Year.docx'),
    'creative_arts_design_b8': ('curriculum_db/creative-arts-design_B8_curriculum_db_clean.json', 'Basic8_Creative_Arts_Design_Lesson_Plans_Full_Year.docx'),
    'french_b8':               ('curriculum_db/french_B8_curriculum_db_clean.json',               'Basic8_French_Lesson_Plans_Full_Year.docx'),
}
...
def audit_docx(stem, docx_name, lessons):
    ...
    if 'B8' not in j: issues.append('title does not say B8')
...
ok = True
for r in rows:
    print(f"[{r['status']}] {r['subject']:26s} {r.get('docx','(json)')}")
