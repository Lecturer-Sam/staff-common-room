...
Every successful download is recorded to `generated_materials` and shown in two
places: the teacher's own "recent downloads" list on the Generate screen, and
the school's "Materials generated by your teachers" section in the workspace.

Rules pin `schoolId` to the caller's own school and `authorId` to the caller, so
history cannot be forged across schools. Full detail in
`docs/SCHOOL_WORKSPACE.md`.

## Not built yet

- **Documents are not stored.** The history is metadata only — filenames and
  parameters, not the files. Re-download would mean persisting to Cloud Storage,
  which needs `storage.rules`.
