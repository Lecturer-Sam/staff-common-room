#!/usr/bin/env python3
"""Generate 180 full-year daily lesson plans for Basic 5 Core-4 subjects:
Mathematics, Science, English, Ghanaian Language. Uses verified B5 DBs.
...
Output: /home/user/{subject}_b5_lessons_enriched.json
"""
import json
...
subjects = [
    {"key": "math", "sid": "mathematics", "name": "Mathematics",
     "plan": [("Monday", "B5.1"), ("Tuesday", "B5.2"), ("Wednesday", "B5.3"),
              ("Thursday", "B5.4"), ("Friday", "B5.1")]},
    {"key": "science", "sid": "science", "name": "Integrated Science",
     "plan": [("Monday", "B5.1"), ("Tuesday", "B5.2"), ("Wednesday", "B5.3"),
              ("Thursday", "B5.4"), ("Friday", "B5.5")]},
    {"key": "english", "sid": "english-language", "name": "English Language",
     # 3 rotating weekly patterns: guarantees all 40 Oral indicators get sessions
     # A (wk 1,4,7,10): Fri=Extensive | B (wk 2,5,8,11): Fri=Conventions | C (wk 3,6,9,12): Wed=Oral, Fri=Conventions
     "patterns": [
         [("Monday", "B5.1"), ("Tuesday", "B5.2"), ("Wednesday", "B5.3"), ("Thursday", "B5.4"), ("Friday", "B5.6")],
         [("Monday", "B5.1"), ("Tuesday", "B5.2"), ("Wednesday", "B5.3"), ("Thursday", "B5.4"), ("Friday", "B5.5")],
         [("Monday", "B5.1"), ("Tuesday", "B5.2"), ("Wednesday", "B5.1"), ("Thursday", "B5.4"), ("Friday", "B5.5")],
     ]},
    {"key": "ghanaian", "sid": "ghanaian-language", "name": "Ghanaian Language",
     "plan": [("Monday", "B5.1"), ("Tuesday", "B5.2"), ("Wednesday", ["B5.3", "B5.6"]),
              ("Thursday", "B5.4"), ("Friday", "B5.5")]},  # odd wk: Writing, even: Extensive
]

GH_STRAND_NAMES = {
    "B5.1": "1. NTI - Listening and Speaking / Oral Language",
    "B5.2": "2. AKENKAN - Reading",
    "B5.3": "3. ATWER\u0190 - Writing",
    "B5.4": "4. ATWER\u0190 - Composition Writing",
    "B5.5": "5. ATWER\u0190 HO NHYEHY\u0190E / Writing Conventions",
    "B5.6": "6. AKENKAN TR\u0190W - Extensive Reading",
}

RPK = {
    "math": "Learners worked with numbers up to 1,000,000, fractions, shapes and data in Basic 1-4 and use money and measures daily.",
    "science": "Learners classified living things, explored materials, systems, energy and their environment through Basic 1-4 practical activities.",
    "english": "Learners can hold simple conversations, read short passages and write structured sentences and paragraphs from Basic 1-4.",
    "ghanaian": "Learners speak the local language daily and built songs, reading, composition and grammar skills through Basic 1-4.",
}

READERS_EN = ["a folk tale book", "a poem collection", "a short story book", "a class chart story",
              "a picture book", "a news article for children"]
...
def build_one(subj):
    path = f"{DBDIR}/{subj['sid']}_B5_curriculum_db_clean.json"
    with open(path) as f:
        cur_db = json.load(f)
    ...
    out = f"/home/user/{subj['key']}_b5_lessons_enriched.json"
