#!/usr/bin/env python3
"""Generate 180 full-year daily lesson plans for Basic 4 Core-4 subjects:
Mathematics, Science, English, Ghanaian Language. Uses verified B4 DBs.
...
Output: /home/user/{subject}_b4_lessons_enriched.json
"""
import json
...
subjects = [
    {"key": "math", "sid": "mathematics", "name": "Mathematics",
     "plan": [("Monday", "B4.1"), ("Tuesday", "B4.2"), ("Wednesday", "B4.3"),
              ("Thursday", "B4.4"), ("Friday", "B4.1")]},
    {"key": "science", "sid": "science", "name": "Integrated Science",
     "plan": [("Monday", "B4.1"), ("Tuesday", "B4.2"), ("Wednesday", "B4.3"),
              ("Thursday", "B4.4"), ("Friday", "B4.5")]},
    {"key": "english", "sid": "english-language", "name": "English Language",
     "plan": [("Monday", "B4.1"), ("Tuesday", "B4.2"), ("Wednesday", "B4.3"),
              ("Thursday", "B4.4"), ("Friday", ["B4.5", "B4.6"])]},  # odd wk: Conventions, even: Extensive
    {"key": "ghanaian", "sid": "ghanaian-language", "name": "Ghanaian Language",
     "plan": [("Monday", "B4.1"), ("Tuesday", "B4.2"), ("Wednesday", ["B4.3", "B4.6"]),
              ("Thursday", "B4.4"), ("Friday", "B4.5")]},  # odd wk: Writing, even: Extensive
]

GH_STRAND_NAMES = {
    "B4.1": "1. NTI - Listening and Speaking / Oral Language",
    "B4.2": "2. AKENKAN - Reading",
    "B4.3": "3. ATWER\u0190 - Writing",
    "B4.4": "4. ATWER\u0190 - Composition Writing",
    "B4.5": "5. ATWER\u0190 HO NHYEHY\u0190E / Writing Conventions",
    "B4.6": "6. AKENKAN TR\u0190W - Extensive Reading",
}

RPK = {
    "math": "Learners used number charts, place-value cards and manipulatives in Basic 1-3 and handle money and measures in daily life.",
    "science": "Learners observed plants, animals, weather, energy and their environment through Basic 1-3 practical activities.",
    "english": "Learners can listen to and give simple oral descriptions, read short texts and write simple sentences from Basic 1-3.",
    "ghanaian": "Learners speak the local language daily and learned songs, reading, writing and grammar basics in Basic 1-3.",
}
...
def build_one(subj):
    path = f"{DBDIR}/{subj['sid']}_B4_curriculum_db_clean.json"
    with open(path) as f:
        cur_db = json.load(f)
    ...
    for term in (1, 2, 3):
        for week in range(1, 13):
            for day_name, slot in subj["plan"]:
