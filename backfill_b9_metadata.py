#!/usr/bin/env python3
"""Backfill B9 Core-4 DBs (math, science, english, ghanaian) with authentic NaCCA CCP
text. All grabs GRADE-SCOPED to BASIC 9 body slices; /JHS-aware code matcher."""
...
BODIES = {
    'mathematics_CCP_B7-B9_draft.pdf': (190705, None),
    'science_CCP_B7-B9.pdf': (192669, None),
    'english_CCP_B7-B9.pdf': (157496, None),
    'ghanaian_language_CCP_B7-B9.pdf': (131416, None),
}

JOBS = [
    ('mathematics', 'B9', 'mathematics_CCP_B7-B9_draft.pdf', {
        1: '1. NUMBER', 2: '2. ALGEBRA', 3: '3. GEOMETRY AND MEASUREMENT', 4: '4. DATA'}),
    ('science', 'B9', 'science_CCP_B7-B9.pdf', {
        1: '1. DIVERSITY OF MATTER', 2: '2. CYCLES', 3: '3. SYSTEMS',
        4: '4. FORCES AND ENERGY', 5: '5. HUMANS AND THE ENVIRONMENT'}),
    ('english-language', 'B9', 'english_CCP_B7-B9.pdf', {
        1: '1. ORAL LANGUAGE (LISTENING AND SPEAKING)', 2: '2. READING',
        3: '3. GRAMMAR USAGE', 4: '4. WRITING', 5: '5. LITERATURE'}),
    ('ghanaian-language', 'B9', 'ghanaian_language_CCP_B7-B9.pdf', {
...
for sid, g, pdf, strand_map in JOBS:
    ...
    bak = os.path.join(BK, os.path.basename(path) + '.pre_b9backfill')
...
for sid, g, n, i, c, s, misses, zero_hits in report:
    ...
    if zero_hits:
        print(f"   ? ZERO hits in B9 body (artifact candidates): {zero_hits[:8]}")
    if misses:
